...
#ifdef MY_ABC_HERE
     void synosmb_debug(int cls, int level, const char *fmt, ...);
#  define SYNOSMB_FOLD(fmt, ...)   synosmb_debug(DBGC_SYNO, 5, __location__": (pid=%u) {{{ "fmt"\n",\
				(unsigned int)getpid(), ##__VA_ARGS__)
#  define SYNOSMB_FOLD_END()   synosmb_debug(DBGC_SYNO, 5, __location__": (pid=%u) }}}\n",\
				(unsigned int)getpid())
#  define SYNOSMB_LOG(level, fmt,...) DEBUGC(DBGC_SYNO, level, (fmt"\n", ##__VA_ARGS__))
#  define SYNOSMB_ERR(fmt,...) DEBUGC(DBGC_SYNO, 0, (fmt"\n", ##__VA_ARGS__))
#else
#  define SYNOSMB_FOLD(...)
#  define SYNOSMB_FOLD_END()
#  define SYNOSMB_LOG(...)
#  define SYNOSMB_ERR(fmt...) 
#  define SYNOSmbSzFsp(...) ""
#endif /* MY_ABC_HERE */
