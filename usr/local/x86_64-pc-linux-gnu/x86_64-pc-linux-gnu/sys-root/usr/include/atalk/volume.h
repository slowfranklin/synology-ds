...
#ifdef MY_ABC_HERE
	int		v_sharestatus;
	BOOL    v_has_remote_mount;
#endif
#ifdef MY_ABC_HERE
	BOOL	v_recycled; // recycle bin is enabled or not
	BOOL	v_recycle_admin_only; // recycle bin is only accessed by Administrators or not
#endif
#ifdef MY_ABC_HERE
    int     v_ronly;
#endif
...
#ifdef MY_ABC_HERE
#define IS_ADEA_VOL(vol) (ISADFS((vol)->v_fstype))
#define IS_HFSEA_VOL(vol) (ISHFSPLUSFS((vol)->v_fstype))
#define IS_FILEEA_VOL(vol) (ISFILEEAFS((vol)->v_fstype))
#endif

#ifdef MY_ABC_HERE
extern void SynoVolumeSetHasRemoteMount(const struct adouble* pad, struct vol* vol);
#endif
