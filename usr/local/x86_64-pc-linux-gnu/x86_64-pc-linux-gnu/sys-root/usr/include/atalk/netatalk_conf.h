...
int SYNOAppleCheckPerm(const char *szVolName, const char *szUser);
int SYNOCheckAddUserHome(PSLIBSZLIST *pplistShareName, PSYNOUSER pUser);
int SYNOAppleLoadVol(AFPObj *obj, const PSLIBSZLIST plistShareName, const PSYNOUSER pUser);
#endif

#ifdef MY_ABC_HERE
#include <synosdk/fs.h>
void SYNOCurrentVolSet(struct vol* volume);
FSTYPE SYNOCurrentVolFSTypeGet();
BOOL SYNOCurrentVolUnixPermGet();
#endif

#ifdef MY_ABC_HERE
int SYNOCurrentVolCaselessGet();
#endif

#ifdef MY_ABC_HERE
BOOL SYNOMediaIndexIsEnable(const char* szPath);
#endif
