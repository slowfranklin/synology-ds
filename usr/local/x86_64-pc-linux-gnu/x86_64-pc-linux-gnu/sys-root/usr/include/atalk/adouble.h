...
#endif
#ifdef MY_ABC_HERE
	char ad_finfo[ADEDLEN_FINDERI];
	int isRemoteMount;
#endif
};

#ifdef MY_ABC_HERE
#define SYNO_ATEADIR_DISK 0
#define SYNO_DOTFILE_DISK 1
#define AD_IS_ATEADIR(ad) (ad->ad_osx_type == SYNO_ATEADIR_DISK)
#define AD_IS_DOTFILE(ad) (ad->ad_osx_type == SYNO_DOTFILE_DISK)
#endif
#ifdef MY_ABC_HERE
#define SYNO_HFSPLUS_DISK 2 /* sync finderinfo & resourcefork to filesystem xattr */
#define AD_IS_HFSPLUS(ad) (ad->ad_osx_type == SYNO_HFSPLUS_DISK)
#define SYNO_FILEEA_DISK 3
#define AD_IS_FILEEAFS(ad) (ad->ad_osx_type == SYNO_FILEEA_DISK)
#endif
...
#ifdef MY_ABC_HERE
#define SYNO_ADFLAGS_LOCKATTR  (1<<13)
#define SYNO_ADFLAGS_RONLY  (1<<16)
#endif
...
#ifdef MY_ABC_HERE
#define ad_getentryoff_hfs(ad,eid)     ((eid == ADEID_RFORK && (ad)->ad_osx_type == SYNO_HFSPLUS_DISK) ? 0 : ad_getentryoff(ad, eid))
#define adedoff_rfork_ea_hfs(ad, eid)     ((eid == ADEID_RFORK && (ad)->ad_osx_type == SYNO_HFSPLUS_DISK) ? 0 : ADEDOFF_RFORK_OSX)
#endif
...
#ifdef MY_ABC_HERE
#define IS_AD_IN_SAME_FS(addst, adsrc) ((adsrc)->ad_osx_type == (addst)->ad_osx_type)
#endif
/* ad_flush.c */
...
#ifdef MY_ABC_HERE
extern const char *ad_path_syno  (const char *path, int adflags);
#endif
...
#ifdef MY_ABC_HERE
extern void SynoADInit(struct adouble *ad, int options, FSTYPE fstype);
#endif
...
#ifdef MY_ABC_HERE
extern int syno_ad_convert_osx(const char *path, const struct stat *sp, int adflags, const char **newpath);
extern int syno_ad_convert_header(struct adouble* pADea, int adflags);
#endif
...
#ifdef MY_ABC_HERE
extern int     reflink_copy_fork(int eid, struct adouble *add, struct adouble *ads, char *buf, size_t buflen);
#endif
...
#ifdef WITH_RECVFILE
#ifdef MY_ABC_HERE
extern ssize_t ad_recvfile(struct adouble *ad, int eid, int sock, off_t off, size_t len, int splice_size, size_t* pRecvCount);
#else
...
#ifdef MY_ABC_HERE
int synoCreateTimeGet(const char* szPath, struct stat* pst, time_t* pTime);
#endif
...
int synoSmbAttrNoADGet(const char *filename, int *pOutput);
void synoSmbAttrGet(const char *filename, struct adouble *ad);
#endif

#ifdef MY_ABC_HERE
int SynoDFPathGet(const char* path, char* szComPath, int cbComPath);
#endif
