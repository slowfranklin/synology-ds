...
#ifdef MY_ABC_HERE
static inline void hci_set_bit_8(int nr, void *addr)
{
	*((uint8_t *) addr + (nr >> 5)) |= (1 << (nr & 31));
}

static inline void hci_clear_bit_8(int nr, void *addr)
{
	*((uint8_t *) addr + (nr >> 5)) &= ~(1 << (nr & 31));
}

static inline int hci_test_bit_8(int nr, void *addr)
{
	return *((uint8_t *) addr + (nr >> 5)) & (1 << (nr & 31));
}
#endif
